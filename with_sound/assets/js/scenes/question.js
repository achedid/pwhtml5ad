class QuestionScene extends Phaser.Scene{
    ...
    displaySetResult(){
      ...
      if(!this.resultIsDisplayed){
          ...
          if(this.correctQuestions == this.max_questions){
            ...
            this.question_times_up.play();
          }

          this.downloadAppButton = this.add.image(0, 200, 'yellow_button');
          this.download_text = this.add.text(-55, 188, 'Download Now!', {fontSize: 20, fontFamily: 'HauracherellNC', color: 'white'});
    ...
    displayQuestion(question){
      ...
      this.question_text = this.add.text(-120, -210, question.description, {fontSize: 20, stroke: 'black', strokeThickness: 2,
                                                                          fontFamily: 'HauracherellNC', align: 'center', boundsAlignH: "center", boundsAlignV: "middle"});
      ...
      question.answers.forEach(function (e, i){
          ...
          var answerText = that.add.text(0, y-15, e.description, {align: 'center', fontSize: 25, stroke: 'black', strokeThickness: 2, fontFamily: 'HauracherellNC'})
          answerText.alpha = 0;
          ...
          answerImage.setInteractive().on('pointerdown', function (event){
                ...
                if(!that.user_answered){
                  ...
                  if(e.correct){
                    ...
                    var correctPoints = that.add.text(oldX - 16, oldY, '+100', {fontSize: 20, fontFamily: 'HauracherellNC', stroke: 'black', strokeThickness: 2});
                      that.question_correct.play();
                  }
                  else{
                    answerImage = that.add.image(oldX, oldY, 'answer2x2r');
                      that.question_incorrect.play();
                  }
    ...
    displayAnswerPopup(correct){
      ...
      if(!this.answerPopupEnabled){
        ...
        this.titleText = this.add.text(-65, -120, title, {fontSize: 20, color: color, fontFamily: 'HauracherellNC'})
        this.pointsText = this.add.text(-110, -20, points + ' pts', {fontSize: 22, color: "#f7c903", fontFamily: 'HauracherellNC'})
        this.rankText = this.add.text(-5, -20, rank, {fontSize: 25, color: "#f7c903", fontFamily: 'HauracherellNC'})
        this.nextPointsText = this.add.text(50, -20, nextPoints + ' pts', {fontSize: 22, color:  "#f7c903", fontFamily: 'HauracherellNC'})
    ...
    preload(){
        ...
        this.load.audio('question_sound', 'assets/sounds/sound_question.mp3');
        this.load.audio('question_correct', 'assets/sounds/sound_correct.mp3');
        this.load.audio('question_incorrect', 'assets/sounds/sound_wrong.mp3');
        this.load.audio('question_times_up', 'assets/sounds/sound_times_up.mp3');

        this.load.audio('winners_sound', 'assets/sounds/sound_winners.mp3');
    ...
    create(){
      ...
      this.question_sound = this.sound.add('question_sound');
      this.question_correct = this.sound.add('question_correct');
      this.question_incorrect = this.sound.add('question_incorrect');
      this.question_times_up = this.sound.add('question_times_up');

      this.q_effect = this.add.image(117, -230, 'q_effect');
      this.question_nb = this.add.text(-55, -263, '1', {fontSize: 20, stroke: 'black', strokeThickness: 2, fontFamily: 'HauracherellNC'});

      this.timer_placeholder = this.add.text(110, -250, '10', {fontSize: 20, stroke: 'black',  strokeThickness: 2, fontFamily: 'HauracherellNC'});
